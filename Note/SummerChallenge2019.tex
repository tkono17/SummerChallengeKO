...
gcc -v
\end{lstlisting}
でgccのバージョンを確認してください。「Binary distributions」のところの同じバージョンのものをダウンロードしてください。

\vspace{0.3cm}
ファイルの展開
\vspace{0.3cm}

自分はホームディレクトリに「local」というディレクトリを作り、ここにインストールします。\\
「local」に先ほどダウンロードしたものをコピーしてきてください。
\begin{lstlisting}
mkdir local
cp (ファイルの場所) ./local/
\end{lstlisting}
...
\begin{lstlisting}
cd local
tar zxvf root_v6.18.00.Linux-ubuntu18-x86_64-gcc7.4.tar.gz
ls
root  root_v6.18.00.Linux-ubuntu18-x86_64-gcc7.4.tar.gz
\end{lstlisting}
root/bin/　にある「thisroot.sh」がROOTを実行するシェルファイルです。\\
\begin{lstlisting}
cd root/bin/
ls
\end{lstlisting}
...
ls -l
\end{lstlisting}
読み取りの権限がなかったら権限を変更します。\\
参考(\url{https://eng-entrance.com/linux-command-chmod}、\url{https://www.server-memo.net/tips/command/chmod/chmod.html})
\begin{lstlisting}
chmod 550 thisroot.sh
\end{lstlisting}
今回は書き込むことはないので、「読み込み」と「実行」を許可します。
...
vim ~/.bash_profile
\end{lstlisting}
のコマンドでエディタを開いたあと以下を追記(usernameは自分のUbuntuのユーザー名に)
\begin{lstlisting}
source ~/home/username/usr/root/bin/thisroot.sh
\end{lstlisting}
