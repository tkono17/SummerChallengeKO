...
class CommandDispatcher
  ...
  COMMANDS = %w(shutdownHV setHV statusHV statusTemp statusInputDAC 
    ...
    exit quit progress stop makeError moveAndRead) + DIRECT_COMMANDS.map(&:to_s)
...
#@vmeEasiroc.sendTriggerValues
  ...
  def moveAndRead(events, name, steps, number, init)
    def originStage
      puts("origin stage")
      system("python3 stage.py -o +")
      sleep(8)
    end
    def moveStage(n)
      printf("move stage for %d pulses \n",n)
      cmd = sprintf("python3 stage.py -ml %d",n)
      system(cmd)
      sleep(5)
    end
    ...
    while i<number.to_i do
      ...
      puts "read: "
      filename = sprintf('%s_%03d', name, i)
