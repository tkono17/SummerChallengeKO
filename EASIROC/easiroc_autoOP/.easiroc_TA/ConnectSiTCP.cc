...
int ContinuousADC_ver2(int socket){
 ...
 // std::ofstream file(file_name.c_str(),std::ios::binary);
  ...
  int step_width = 0;
  std::cout << "Input step width(um) : " << std::flush;
  std::cin >> step_width;
  int Step = step_width/2;
  std::stringstream MoveStage;
  MoveStage << "python /home/smcha2019/SummerChallengeKO/code_for_stage/stage.py -mr " << Step;
  std::string MoveStep = MoveStage.str();
  time_t t1,t2;
...
for(int move_count=0;move_count<3;move_count++){
  ...
  FileName << file_name << "_" << move_count << ".dat";
  std::string CurrentFileName = FileName.str();
  //std::sprintf(FileName, "file_name_%03d.dat", move_count);
  std::ofstream file(CurrentFileName.c_str(),std::ios::binary);
  ...
  system("python /home/smcha2019/SummerChallengeKO/code_for_stage/stage.py -ml 500000");
  sleep(5);
  system("python /home/smcha2019/SummerChallengeKO/code_for_stage/stage.py -mr 1000");
  sleep(3);
  t2 = time(NULL);
